...
class FeedCard extends ConsumerStatefulWidget {
  ...
  const FeedCard({Key? key, this.onTapNavigate = true, this.feedData, this.feedType, this.showCommentModal = false, this.id}) : super(key: key);
...
class _FeedCardState extends ConsumerState<FeedCard> {
  ...
  Widget build(BuildContext context) {
    return widget.feedData!.activityType=="feed" ||widget.feedData!.activityType=="share" || widget.feedData!.isAdPost==false ||
    widget.feedData!.campaign==null?
  //  Container():
    GestureDetector(
      onTap: () {
        if (widget.onTapNavigate) {
          ref.read(feedDetailsProvider.notifier).fetchFeedDetails(widget.feedData!.id!);
          Navigator.of(context, rootNavigator: true).push((CupertinoPageRoute(builder: (context) => const FeedDetailsScreen())));
        }
      },
      child: Container(
        margin: EdgeInsets.only(left: KSize.getWidth(context, 0), right: KSize.getWidth(context, 0), bottom: 10),
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              widget.feedData!.isAdPost!
                                  ? Text('Sponsored', style: KTextStyle.bodyText2.copyWith(color: KColor.black54, fontSize: 12, letterSpacing: 0))
                                  : Wrap(
                                      crossAxisAlignment: WrapCrossAlignment.center,
                                      children: [
                                        if (widget.feedData!.activityType == 'group' && widget.feedType != FeedType.GROUP)
                                          Text(
                                            '${widget.feedData!.user!.firstName} ${widget.feedData!.user!.lastName}',
                                            style: KTextStyle.bodyText2
                                                .copyWith(color: KColor.black54, fontSize: 12, letterSpacing: 0, fontWeight: FontWeight.bold),
                                          ),
                                        if (widget.feedData!.activityType == 'event' && widget.feedType != FeedType.EVENT)
                                          Text(
                                            '${widget.feedData!.user!.firstName} ${widget.feedData!.user!.lastName}',
                                            style: KTextStyle.bodyText2
                                                .copyWith(color: KColor.black54, fontSize: 12, letterSpacing: 0, fontWeight: FontWeight.bold),
                                        ...
                                        if (widget.feedData!.activityType == 'group' || widget.feedData!.activityType == 'event')
                                          Visibility(
                                            visible: widget.feedType != FeedType.GROUP && widget.feedType != FeedType.EVENT,
                                            child: Padding(
                                        ...
                                        Text(
                                          DateTimeService.timeAgoLocal(widget.feedData!.createdAt.toString()) ?? '',
                ...
                PopupMenuButton(
                  ...
                  onSelected: (selected) {
                    ...
                    //       .push(CupertinoPageRoute(builder: (context) => ReportFeedScreen(widget.feedData, "feed")));
                    // }
                    // if (selected == 'edit') {
                    //   Navigator.of(context, rootNavigator: true)
                    //       .push(CupertinoPageRoute(builder: (context) => EditFeedScreen(feed: widget.feedData, feedType: widget.feedType)));
                    // }
                    // if (selected == 'save') {
                    //   widget.feedData?.savedPosts != null
                    //       ? ref.read(feedProvider.notifier).unsaveFeed(widget.feedData!.id!, widget.feedType!)
                    //       : ref.read(feedProvider.notifier).saveFeed(widget.feedData!.id!, widget.feedType!);
                    // }
                    if (selected == 'delete') {
                      _deleteConfirmationDialog(widget.feedData!, widget.feedType!);
                    }
                    ...
                    // }
                    // if (selected == 'hide') {
                    //   _hideConfirmationDialog(widget.feedData!, widget.feedType!);
                    // }
                  },
                  color: AppMode.darkMode ? KColor.feedActionCircle : KColor.appBackground,
                  shape: const RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(6))),
                  offset: const Offset(0, -10),
                  icon: Icon(MaterialIcons.more_vert, color: KColor.black87),
                  itemBuilder: (_) => <PopupMenuItem<String>>[
                 // if( widget.feedData!.userId != getIntAsync(USER_ID))
                    // PopupMenuItem<String>(
                    //     value: "report",
                    //     child: Text('Report', style: KTextStyle.subtitle2.copyWith(color: KColor.black87, fontWeight: FontWeight.normal))),
                    // PopupMenuItem<String>(
                    //     value: "save",
                    //     child: Text(widget.feedData!.savedPosts != null ? 'Unsave' : 'Save',
                    //         style: KTextStyle.subtitle2.copyWith(color: KColor.black87, fontWeight: FontWeight.normal))),
                    // if (widget.feedData!.isFeedEdit!)
                    //   if (widget.feedData!.poll == null || (widget.feedData!.poll != null && widget.feedData!.poll!.voteByAnyOne == 'no'))
                    //     PopupMenuItem<String>(
                    //         value: "edit",
                    //         child: Text('Edit', style: KTextStyle.subtitle2.copyWith(color: KColor.black87, fontWeight: FontWeight.normal))),
                    //if (widget.feedData!.isFeedEdit!)
                      PopupMenuItem<String>(
                          value: "delete",
                          child: Text('Delete', style: KTextStyle.subtitle2.copyWith(color: KColor.black87, fontWeight: FontWeight.normal))),
                    ...
                    //   PopupMenuItem<String>(
                    //       value: "hide",
                    //       child: Text('Hide', style: KTextStyle.subtitle2.copyWith(color: KColor.black87, fontWeight: FontWeight.normal))),
                  ],
                ),
              ],
            ),
                ...
                ? widget.feedData!.share == null
                    ? Padding(
                        padding: EdgeInsets.symmetric(
                          horizontal: widget.feedData?.isBackground == 0 ? 10 : 0,
                        ),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            ...
                            SizedBox(height: KSize.getHeight(context, 10)),
                            const KContentUnvailableComponent(isUnavailablePost: true),
                          ],
                        ),
                      )
                    : Container(
                        padding: EdgeInsets.symmetric(horizontal: widget.feedData?.isBackground == 0 ? 10 : 0),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            if (widget.feedData!.feedTxt != null)
                      ...
                      children: [
                        if (widget.feedData?.feedTxt != null)
                          widget.feedData?.isBackground == 0
                              ? InkWell(
                                  onTap: () {
                                    if (widget.feedData?.metaData?.linkMeta != null) {
                                      launch(widget.feedData!.metaData!.linkMeta!.url!);
                                    }
                                  },
                                  child: Text(
                                    widget.feedData!.feedTxt ?? '',
                                    style: KTextStyle.bodyText2.copyWith(color: KColor.black, height: KSize.getHeight(context, 1.5)),
                                  ),
                                )
                              : InkWell(
                                  onTap: () {
                                  ...
                                  child: Container(
                                    ...
                                    decoration: BoxDecoration(
                                        gradient: KColor.gradientsColor[
                                            KColor.feedBackGroundGradientColors.indexWhere((element) => element == widget.feedData?.bgColor) + 2]),
                                    child: Text(
                                      widget.feedData!.feedTxt ?? '',
                        ...
                        */
                        ...
                        if (widget.feedData!.fileType == 'image')
                          widget.feedData!.files!.length == 1
                              ? GestureDetector(
                                  onTap: () {
                                    print(widget.feedData!.files![0].fileLoc);
                                    Navigator.of(context)
                                        .push(CupertinoPageRoute(builder: (context) => KImageView(url: widget.feedData!.files![0].fileLoc)));
                                  },
                                  child: Container(
                                    ...
                                    child: ClipRRect(
                                      ...
                                      child: Image.network(
                                        ...
                                        cacheHeight: KSize.getHeight(context, 1000).ceil(),
                                        cacheWidth: (MediaQuery.of(context).size.width * 2).ceil(),
                                        fit: BoxFit.cover,
                                      ),
                                    ),
                                  ),
                                                  ...
                                                  child: Stack(
                                                    children: [
                                                      Image.network(
                                                        ...
                                                        height: KSize.getHeight(context, 150),
                                                        width: MediaQuery.of(context).size.width * 0.4,
                                                        cacheHeight: KSize.getHeight(context, 600).ceil(),
                                                        cacheWidth: (MediaQuery.of(context).size.width * 1.2).ceil(),
                                                        ...
                                                        errorBuilder: (BuildContext context, Object exception, StackTrace? stackTrace) {
                                                          return Icon(Icons.error, color: AppMode.darkMode ? KColor.white : KColor.black);
                                                        },
                                                      ),
                                                      if (index == 3 && widget.feedData!.files!.length > 4)
                                                        Container(
                                                          ...
                                                          decoration: BoxDecoration(color: KColor.blackConst.withOpacity(0.4)),
                                                          child: Center(
                                                            child: Text(
                                                              '+${widget.feedData!.files!.length - 3}',
                                                              style: KTextStyle.headline5.copyWith(color: KColor.whiteConst),
                                                            ),
                                                          ),
                                                        ),
                                                    ],
            ...
            */
            ...
            Padding(
              padding:
                  EdgeInsets.symmetric(vertical: (widget.feedData!.likeCount == 0 && widget.feedData!.commentCount == 0) ? 0 : 10, horizontal: 10),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  ...
                  if (widget.feedData!.commentCount != 0)
                    GestureDetector(
                      ...
                      child: Text(
                        widget.feedData!.commentCount == 1 ? '1 comment' : '${widget.feedData!.commentCount} comments',
                        style: KTextStyle.bodyText2.copyWith(color: KColor.black54, fontWeight: FontWeight.w500),
                      ),
                    ),
                ],
            ...
            Container(
                padding: EdgeInsets.symmetric(
                  vertical: (widget.feedData!.likeCount == 0 && widget.feedData!.commentCount == 0) ? 0 : 5,
                ),
                child: Divider(height: 0, color: KColor.black.withOpacity(0.2))),
            ...
            //                 feedId: widget.feedData!.id,
            //                 feedType: widget.feedType,
            //                 isLike: widget.feedData?.like == null ? true : false,
            //                 reactionType: 'LIKE',
            ...
            //         child: InkWell(
            //           onTap: () {
            //             _showOptionsModal(context, widget.feedData, widget.feedType, widget.id);
            //           },
            //           child: Padding(
            //             padding: const EdgeInsets.only(top: 8, bottom: 10),
            //             child: Row(
  ...
  _deleteConfirmationDialog(FeedModel feed, FeedType feedType) async {
    return (await showPlatformDialog(
      ...
      builder: (context) => ConfirmationDialogContent(
          ...
          onPressedCallback: () {
            Navigator.pop(context);
            KDialog.kShowDialog(context, const ProcessingDialogContent('Deleting...'), barrierDismissible: false);
            ref.read(feedProvider.notifier).deleteFeed(feed: feed, feedType: feedType);
          }),
    ));
  }
  ...
  _hideConfirmationDialog(FeedModel feed, FeedType feedType) async {
    return (await showPlatformDialog(
      ...
      builder: (context) => ConfirmationDialogContent(
          ...
          onPressedCallback: () {
            Navigator.pop(context);
            KDialog.kShowDialog(context, const ProcessingDialogContent('Hiding Post...'), barrierDismissible: false);
            ref.read(feedProvider.notifier).hideFeed(feed: feed, feedType: feedType);
          }),
    ));
  }
